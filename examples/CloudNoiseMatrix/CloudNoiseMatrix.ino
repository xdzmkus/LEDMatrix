...
void changeEffect()
{
	...
	effect = new NoiseMatrixLedEffect(new BottomLeftCorner<MATRIX_W, MATRIX_H>, leds, NUM_LEDS, speed, CloudColors_p, zoom);
}
...
void setup()
{
	...
	Serial.println(F("Cloud NoiseMatrix effect"));

	setupLED();

	tickerEffects.attach(EFFECT_DURATION_SEC, handleTimer);

	effect = new NoiseMatrixLedEffect(new BottomLeftCorner<MATRIX_W, MATRIX_H>, leds, NUM_LEDS, 15, CloudColors_p);
}
